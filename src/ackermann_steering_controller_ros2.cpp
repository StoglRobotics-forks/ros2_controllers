...
controller_interface::CallbackReturn AckermannSteeringControllerRos2::on_init()
{
  try {
    param_listener_ =
      std::make_shared<ackermann_steering_controller_ros2::ParamListener>(get_node());
  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  // rt_odom_state_publisher_->msg_.twist =
...
void AckermannSteeringControllerRos2::reference_callback(
  const std::shared_ptr<ControllerReferenceMsg> msg)
{
...
AckermannSteeringControllerRos2::command_interface_configuration() const
{
...
controller_interface::InterfaceConfiguration
AckermannSteeringControllerRos2::state_interface_configuration() const
{
...
AckermannSteeringControllerRos2::on_export_reference_interfaces()
{
...
AckermannSteeringControllerRos2::update_reference_from_subscribers()
{
  auto current_ref = *(input_ref_.readFromRT());
  // const auto age_of_last_command = time - (*current_ref)->header.stamp;
  const auto age_of_last_command = rclcpp::Duration::from_seconds(0);
