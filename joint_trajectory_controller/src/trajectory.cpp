  ...
  const bool search_monotonically_increasing)
{
  ...
  // whole animation has played out - but still continue interpolating and smoothing
  ...
  if (last_point.effort.empty())
  {
    last_point.effort.resize(dofs, 0.0);
  }
  ...
  const double delta_t)
{
  ...
  {
    first_state.effort.assign(dofs, 0.0);
  }
  if (second_state.effort.empty())
  {
    second_state.effort.assign(dofs, 0.0);
  }
