...
#include "gripper_io_controller_parameters.hpp"
#include "gripper_io_controller/visibility_control.h"
...
class GripperIOController : public controller_interface::ControllerInterface
{
public:
  GRIPPER_IO_CONTROLLER__VISIBILITY_PUBLIC
  GripperIOController();
  gripper_io_controller::Params params_;
  ...
  // std::unordered_map<std::string, double> command_ios_open;
  // std::unordered_map<std::string, double> command_ios_close;
  // std::unordered_map<std::string, double> state_ios_open;
  // std::unordered_map<std::string, double> state_ios_close;
  ...
  using IoGripperSensorMsg = control_msgs::msg::IoGripperSensor;
...
protected:
  std::shared_ptr<gripper_io_controller::ParamListener> param_listener_;
  ...
  using SensorPublisher = realtime_tools::RealtimePublisher<IoGripperSensorMsg>;
  ...
  rclcpp::Publisher<ConfigJointMsg>::SharedPtr c_publisher_;
  std::unique_ptr<ConfigPublisher> config_publisher_;
  ...
  std::vector<rclcpp::Publisher<IoGripperSensorMsg>::SharedPtr> sens_publishers_;
  std::vector<std::unique_ptr<SensorPublisher>> sensor_publishers_;
...
private:
  ...
  gripper_io_controller::Params::ConfigurationSetup::MapConfigurations conf_it_;
