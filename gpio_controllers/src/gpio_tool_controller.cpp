  ...
  const rclcpp::Time & time, const rclcpp::Duration & /*period*/)
{
  switch (tool_action())
  {
    ...
    case ToolAction::CANCELING:
    {
      ...
      // process_tool_action_request()); only force HALTED if that hasn't happened since we
      // entered this case.
  ...
  const std::string & config_name)
{
  ...
  // process_tool_action_request() above for why a plain load-then-store isn't enough here.
  while (true)
  {
    uint16_t seen = tool_state_.load();
